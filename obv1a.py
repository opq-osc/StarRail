...
async def main():
    ...
    await site.start()
    asyncio.create_task(schedule_sign_in())
...
async def Api_GetGroupListB(botqq):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, headers=headers, json=payload) as response:
            ...
            print(response_json)
    group_lists = response_json["ResponseData"]["GroupLists"]
    data = []
    for group in group_lists:
        group_name = group.get("GroupName", "")
        # 不需要进行编码解码操作,直接使用group_name
        data.append({
            "group_create_time": group.get("CreateTime", 0),
            "group_id": group.get("GroupCode", ""),
            "group_level": group.get("GroupLevel", 0),
            "group_memo": group.get("GroupMemo", ""),
            "group_name": group_name,
            "max_member_count": group.get("MaxMemberCount", 500),
...
async def call_api_from_dict(message):
    async with aiohttp.ClientSession() as session:
        ...
        if action == 'get_group_list':
            botqq = message['params']['botqq']
            task = asyncio.create_task(Api_GetGroupListB(botqq))
            result = await task
            await send_to_ws_b(result,botqq)
...
async def schedule_sign_in():
    while True:
        ...
        # 将消息转换为WebSocket消息格式
        ws_msg = aiohttp.WSMessage(aiohttp.WSMsgType.TEXT, json.dumps(msg), None)
        # 将消息发送到recv_message_b函数
        await recv_message_b(ws_msg)
        # 等待20小时
